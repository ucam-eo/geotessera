"""Offline tests for remote-style Zarr I/O and build state."""
...
pytestmark = pytest.mark.filterwarnings("ignore:Object at .* is not recognized")
...
def test_npy_window_reads(tmp_path: Path):
    ...
    for r0, r1, c0, c1 in windows:
        local = remote.read_npy_window(tmp_path / "emb.npy", r0, r1, c0, c1)
        over_url = remote.read_npy_window(url(tmp_path / "emb.npy"), r0, r1, c0, c1)
        assert_check(
    ...
    assert_check(
        ...
        np.array_equal(
            remote.read_npy_window(url(tmp_path / "scales.npy"), 3, 9, 2, 11),
            scales[3:9, 2:11],
        ),
    )

    # A window past the end of the array clamps rather than over-reading.
    clamped = remote.read_npy_window(url(tmp_path / "emb.npy"), 35, 60, 0, 30)
    assert_check("npy window past EOF clamps to array height", clamped.shape[0] == 5)


def test_geotiff_window_reads(tmp_path: Path):
    import rasterio
    ...
    assert_check(
        ...
        np.array_equal(
            remote.read_tiff_window(tmp_path / "lm.tiff", 2, 10, 1, 9),
            remote.read_tiff_window(url(tmp_path / "lm.tiff"), 2, 10, 1, 9),
        ),
    )
    assert_check(
        "tiff window matches source",
        np.array_equal(
            remote.read_tiff_window(url(tmp_path / "lm.tiff"), 0, 6, 0, 6),
            np.zeros((6, 6), dtype=np.uint8),
        ),
    )


def test_store_location_round_trip(tmp_path: Path):
    for label, loc in [
        ("local", StoreLocation(str(tmp_path / "store_local"))),
        ("url", StoreLocation(url(tmp_path / "store_url"))),
    ]:
        assert_check(
            f"{label} store reports remote correctly", loc.is_remote == (label == "url")
        )
        assert_check(f"{label} store missing object absent", not loc.exists("nope.bin"))
        loc.write_bytes(b"hello", "sub", "a.bin")
        assert_check(f"{label} store write creates parents", loc.exists("sub", "a.bin"))
        assert_check(
            f"{label} store read round-trips",
            loc.read_bytes("sub", "a.bin") == b"hello",
        )
        listed = loc.listdir("sub")
        assert_check(f"{label} store lists children", len(listed) == 1)
        ...
        assert_check(f"{label} store remove deletes", not loc.exists("sub", "a.bin"))
...
def test_permission_denied_probes(monkeypatch: pytest.MonkeyPatch):
    ...
    logger = logging.getLogger("geotessera.remote")
    monkeypatch.setattr(logger, "level", logging.ERROR)

    real_get_fs = remote.get_fs
    monkeypatch.setattr(
        remote,
        "get_fs",
        lambda loc, so=None: (
            _DenyingFS() if remote.is_url(loc) else real_get_fs(loc, so)
        ),
    )
    denied = "s3://bucket/store.zarr/zarr.json"
    with pytest.raises(PermissionError):
        remote.exists(denied)
    assert remote.exists(denied, on_denied=False) is False
    assert remote.exists(denied, on_denied=True) is True

    with pytest.raises(PermissionError):
        remote.listdir("s3://bucket/store.zarr.build/_registry")
    assert remote.listdir("s3://bucket/x", on_denied=[]) == []

    from geotessera.zarr import _read_parquet_at
...
def test_legacy_registry_merge(tmp_path: Path):
    def tile(lon, lat, zone=31):
        return TileInfo(
            lon=lon,
            lat=lat,
    ...
    store = StoreLocation(str(tmp_path / "reg_store"))
    assert_check(
        "no merged registry before consolidation", written(store, 2024, 31) == set()
    )
    ...
    assert_check("merge folds every zone into the root registry", n == 3)
    assert_check(
        "merged registry lands in the state sibling",
        store.state.exists("_registry.parquet"),
    )
    assert_check(
        "nothing written into the store itself", not store.exists("_registry.parquet")
    )
    assert_check(
        "state sibling sits next to the store", store.state.url == store.url + ".build"
    )
    assert_check(
    ...
    url_store = StoreLocation(url(tmp_path / "reg_store_url"))
    write_legacy_part(url_store, [(0.05, 52.05)], 2024, 31)
    write_legacy_part(url_store, [(-0.05, 52.05)], 2024, 30)
    assert_check("merge over a url location", merge_tile_registry(url_store) == 2)
    assert_check(
    ...
    legacy = StoreLocation(str(tmp_path / "legacy_store"))
...
def test_shard_index_preserves_neighbours(tmp_path: Path):
    ...
    assert_check(
        "shard coords derived per tile", shard_coords_for_tiles([old], grid) == {(0, 0)}
    )
    assert_check(
    ...
    assert_check(
        "only touched shards are rebuilt", [(s.sr, s.sc) for s in specs] == [(0, 0)]
    )
    assert_check(
    ...
    assert_check("unrestricted index covers every shard", len(unrestricted) == 2)


def test_extend_store_time_axis(tmp_path: Path):
    import zarr
    from zarr.codecs import BloscCodec

    from geotessera.zarr import extend_store

    ext = StoreLocation(str(tmp_path / "ext_store"))
    ...
    for zname in ("utm30", "utm31"):
        ...
        t = grp.create_array(
            "time", shape=(1,), dtype=np.int32, dimension_names=["time"]
        )
        t[:] = [2024]
    ...
    try:
        ...
        assert_check("extend refuses a store without stretch arrays", False)
    ...
    for zname in ("utm30", "utm31"):
        create_stretch_arrays(
            root[zname], n_years=1, k=50, n_shard_rows=1, n_shard_cols=1
        )

    assert_check("extend adds the year to every zone", extend_store(ext, [2026]) == 2)
    assert_check(
    ...
    assert_check("time axis grew", [int(v) for v in g31["time"][:]] == [2024, 2026])
    assert_check("arrays grew along time", g31["embeddings"].shape[0] == 2)
    assert_check(
    ...
    assert_check("extend is idempotent", extend_store(ext, [2026]) == 0)

    try:
        extend_store(ext, [2020])
        assert_check("inserting an earlier year refused", False)
    except ValueError as e:
        assert_check("inserting an earlier year refused", "only be appended" in str(e))

    assert_check("extend appends a further year", extend_store(ext, [2027]) == 2)


def test_stretch_statistics(tmp_path: Path):
    from geotessera.zarr import (
        STRETCH_ARRAY_NAMES,
        create_stretch_arrays,
    ...
    assert_check("stats count exact", sst["n"] == int(svalid.sum()))
    assert_check(
    ...
    assert_check("stats additive across shards", ha["n"] + hb["n"] == sst["n"])
    assert_check(
    ...
    me, _merged_scales = merge_stretch_samples([heavy, light], 300, seed=4)
    assert_check("merge respects capacity", len(me) == 300)
    assert_check(
    ...
    assert_check("sentinel scales excluded from N", jst["n"] == 64 * 64 - 64 - 2)
    assert_check("junk-free sums stay finite", bool(np.isfinite(jst["sum"]).all()))
    assert_check("junk-free products stay finite", bool(np.isfinite(jst["prod"]).all()))
    assert_check(
    ...
    zs = zarr.open_group(str(tmp_path / "stats.zarr"), mode="w", zarr_format=3)
    ...
    assert_check(
        "stats fold additively", int(zs["stretch_stats_count"][0]) == 2 * sst["n"]
    )
    ...
    assert_check("other year untouched", int(zs["stretch_stats_count"][1]) == 0)


def test_preview_work_list_is_sparse(tmp_path: Path):
    from geotessera.zarr import GLOBAL_CHUNK, _chunks_for_shards
    ...
    assert_check("shard footprint yields a small chunk set", 0 < len(chunks) < 200)
    assert_check("an ordinary footprint needs one region", len(regions) == 1)
    ...
    both, _ = _chunks_for_shards(
        {(0, 0), (200, 0)}, 32631, transform31, (1011712, 8192)
    )
...
def test_storage_options_and_source_layout(monkeypatch: pytest.MonkeyPatch):
    for var in ("AWS_ENDPOINT_URL", "AWS_DEFAULT_REGION", "AWS_REGION", "AWS_PROFILE"):
        monkeypatch.delenv(var, raising=False)

    assert_check(
        "no options when nothing configured", remote.build_storage_options() is None
    )
    assert_check(
    ...
    monkeypatch.setenv("AWS_ENDPOINT_URL", "https://from-env.example")
    assert_check(
    ...
    assert_check(
        ...
        s
        == "s3://bucket/tessera/npy/v1/2024/grid_0.05_52.05/grid_0.05_52.05_scales.npy",
    )
    assert_check(
    ...
    assert_check("url source reports remote", src.is_remote)
    ...
    assert_check(
        ...
        e11
        == "s3://bucket/tessera/npy/v1.1-cam/2024/grid_0.05_52.05/grid_0.05_52.05.npy"
        and s11.endswith(
            "npy/v1.1-cam/2024/grid_0.05_52.05/grid_0.05_52.05_scales.npy"
        ),
    )
    assert_check(
...
def test_antimeridian_preview_work_list(tmp_path: Path):
    ...
    wrap_cols, _wrap_span, wrap_regions, wrap_chunks = _shard_cols(
    ...
    assert_check(
        "antimeridian footprint splits into two regions", len(wrap_regions) == 2
    )
    wrap_slots = sum(
        ((b - a) // GLOBAL_CHUNK) * ((d - c) // GLOBAL_CHUNK)
        for a, b, c, d in wrap_regions
    )
    ...
    mid_cols, mid_span, mid_regions, _mid_chunks = _shard_cols(
        32660, 500_000.0, 500_000.0
    )
    assert_check("mid-zone shard stays contiguous", mid_span == len(mid_cols) - 1)
    assert_check("mid-zone shard spans few columns", len(mid_cols) < 20)
    assert_check("mid-zone shard needs one region", len(mid_regions) == 1)


def test_global_preview_pyramid(tmp_path: Path):
    ...
    for label, dest_loc in (
        ("local path", str(tmp_path / "pyr_local.zarr")),
        ("file:// url", url(tmp_path / "pyr_url.zarr")),
    ):
        ...
        assert_check(
            ...
            int(
                dest.open_group(mode="r", zarr_format=3)["global_rgb/1/rgb"][
                    r0 // 2, c0 // 2, 0
                ]
            )
            == 200,
        ...
        elsewhere = StoreLocation.resolve(
            dest_loc, None, url(tmp_path / f"state_{label.split()[0]}")
        )
    ...
    cmp_a = StoreLocation.resolve(str(tmp_path / "coarse_rect.zarr"))
    cmp_b = StoreLocation.resolve(str(tmp_path / "coarse_set.zarr"))
    ...
    assert_check("chunk-driven coarsening matches the rectangle sweep exactly", same)
    ...
    assert_check("coarsened data lands where it should", all(v == 200 for v in got))
    # The rectangle spanning both clusters is ~32x21 chunks; the chunk set is 4.
    rect_tiles = ((max(rows) - min(rows) + 1) // 2 + 1) * (
        (max(cols) - min(cols) + 1) // 2 + 1
    )
    assert_check(
    ...
    race_dest = StoreLocation.resolve(str(tmp_path / "pyr_race.zarr"))
    ...
    with warnings.catch_warnings():
        ...
        zarr.consolidate_metadata(
            StoreLocation.resolve(url(tmp_path / "pyr_url.zarr")).as_zarr_store()
        )
    assert_check(
        "pyramid consolidates over a url",
        StoreLocation.resolve(url(tmp_path / "pyr_url.zarr")).exists(
            "zarr.json", on_denied=False
        ),
    )


def test_nested_depths_and_preview_source(tmp_path: Path):
    from geotessera.zarr import (
        N_BANDS,
        SHARD_SIZE,
        _create_zone_group,
    ...
    assert_check(
        "depth-128 keeps the 32px inner chunk", depth_inner_chunk(N_BANDS) == 32
    )
    assert_check("depth-16 widens to 64px", depth_inner_chunk(16) == 64)
    assert_check("depth-4 widens to 128px", depth_inner_chunk(4) == 128)
    assert_check(
    ...
    assert_check(
        "full depth keeps the base array name",
        depth_array_name(N_BANDS) == "embeddings",
    )
    assert_check(
        "nested depth gets a suffixed name", depth_array_name(4) == "embeddings_d4"
    )
    assert_check("nested depth gets its own band dim", depth_band_dim(4) == "band_d4")
    assert_check(
        "full depth keeps the plain band dim", depth_band_dim(N_BANDS) == "band"
    )
    assert_check(
    ...
    for bad_version in ("1.0", "1.1"):
        try:
            ...
            refused = True
        assert_check(f"depths refused for dataset v{bad_version}", refused)
    ...
    assert_check(
        "no depths requested is always fine",
        validate_matryoshka_depths((), "1.0") == (),
    )

    for bad, why in [
        ((0,), "below 1"),
        ((4, 128), "at the full depth"),
        ((200,), "above"),
    ]:
        try:
            ...
            refused = True
        assert_check(f"depth {why} is refused", refused)

    # A real store: create a zone group with depths and write one shard.
    depth_root = StoreLocation.resolve(url(tmp_path / "depths.zarr"))
    ...
    assert_check(
        "store_depths reads back the declared depths",
        store_depths(depth_root) == (4, 16),
    )
    empty_root = StoreLocation.resolve(url(tmp_path / "empty.zarr"))
    empty_root.open_group(mode="w", zarr_format=3)
    assert_check(
        "a store with no declaration has no depths",
        store_depths(empty_root) == (),
    )

    import geotessera.zarr as _z
    from geotessera.zarr import RGB_PREVIEW_BANDS, preview_source_array
...
def test_seam_aware_point_reads(tmp_path: Path):
    import xarray as xr
    ...
    assert_check("just east of a seam looks west", _seam_neighbours(138.02) == [53])
    assert_check("just west of a seam looks east", _seam_neighbours(137.98) == [54])
    ...
    assert_check(
        ...
        all(
            _seam_neighbours(138.0 + d) for d in (-0.05, -0.03, -0.01, 0.01, 0.03, 0.05)
        ),
    )
    ...
    assert_check("zone 1 wraps west to 60", _seam_neighbours(-179.98) == [60])
    assert_check("zone 60 wraps east to 1", _seam_neighbours(179.98) == [1])
...
def test_landmask_free_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from geotessera.zarr import (
        N_BANDS,
        SHARD_SIZE,
        _create_zone_group,
    ...
    class _FakeRegistry:
        ...
        def __init__(self, emb, land):
            ...
            idx = pd.MultiIndex.from_tuples(
                [(y, round(lo * 100), round(la * 100)) for y, lo, la in emb],
    ...
    assert_check(
        "landmask grid uses the landmask tiles",
        sorted(by_land[31]) == [(3.05, -20.05), (3.05, 10.05)],
    )
    assert_check(
    ...
    assert_check("embedding tiles are deduplicated across years", len(by_emb[31]) == 2)

    # The flag round-trips through a store root, defaulting to "masked".
    lm_root = StoreLocation.resolve(url(tmp_path / "nolm.zarr"))
    _r = lm_root.open_group(mode="w-", zarr_format=3, use_consolidated=None)
    assert_check(
        "a store says masked unless told otherwise",
        store_uses_landmask(lm_root) is True,
    )
    _r.attrs["geoemb:landmask"] = False
    assert_check(
        "a landmask-free store reports itself", store_uses_landmask(lm_root) is False
    )

    # Drive a real shard write both ways and count landmask reads.
    import geotessera.zarr as _z
    from geotessera.zarr import ShardSpec, ShardTileOverlap
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            lm_calls.append(args[0]),
            np.zeros((TW, TW), np.uint8),
        )[1],
    )
    monkeypatch.setattr(
        remote,
        "read_npy_window",
        lambda path, r0, r1, c0, c1, storage_options=None: (
            np.ones((r1 - r0, c1 - c0, N_BANDS), np.int8)
            if "emb" in str(path)
            else np.full((r1 - r0, c1 - c0), np.float32(0.05))
        ),
    )
    for use_landmask in (True, False):
        lm_calls.clear()
        buf = np.zeros((N_BANDS, SHARD_SIZE, SHARD_SIZE), np.int8)
        sbuf = np.full((SHARD_SIZE, SHARD_SIZE), np.float32("inf"))
        _fill_and_write_shard(spec2, zone2, buf, sbuf, None, 0, (), use_landmask)
        got = np.asarray(zone2["scales"][0, 0, 0])
        if use_landmask:
            assert len(lm_calls) == 1
            assert bool(np.isnan(got))
        else:
            assert lm_calls == []
            assert float(got) == np.float32(0.05)
    ...
    monkeypatch.setattr(
        ...
        lambda path, r0, r1, c0, c1, storage_options=None: (
            np.ones((r1 - r0, c1 - c0, N_BANDS), np.int8)
            if "emb" in str(path)
            else np.full((r1 - r0, c1 - c0), np.float32(1e6))
        ),
    )
    buf = np.zeros((N_BANDS, SHARD_SIZE, SHARD_SIZE), np.int8)
    sbuf = np.full((SHARD_SIZE, SHARD_SIZE), np.float32("inf"))
    _fill_and_write_shard(spec2, zone2, buf, sbuf, None, 0, (), False)
    assert bool(np.isnan(np.asarray(zone2["scales"][0, 0, 0])))


def test_zone_stretch_blending():
    from geotessera.zarr import (
        _zone_centre_lon,
        blend_stretches,
        stretch_for_lon,
    ...
    assert_check("zone centre is the central meridian", _zone_centre_lon(31) == 3.0)
    assert_check("zone 1 centre is -177", _zone_centre_lon(1) == -177.0)

    mid = blend_stretches(_st(0.0), _st(10.0), 0.5)
    assert_check("an even blend is the midpoint", mid["min"] == [5.0] * 3)
    assert_check("and blends the cdf breakpoints too", mid["cdf"][0] == [5.0] * 4)
    assert_check(
        "weight 0 returns the first operand",
        blend_stretches(_st(2.0), _st(9.0), 0.0)["min"] == [2.0] * 3,
    )
    assert_check(
        "weight 1 returns the second",
        blend_stretches(_st(2.0), _st(9.0), 1.0)["min"] == [9.0] * 3,
    )
    assert_check(
        "a missing operand passes the other through",
        blend_stretches(None, _st(4.0), 0.5)["min"] == [4.0] * 3,
    )
    assert_check(
        "both missing yields nothing", blend_stretches(None, None, 0.5) is None
    )
    ...
    steps = [abs(b - a) for a, b in pairwise(samples)]
    assert_check("crossing a boundary is continuous", max(steps) < 0.01)
    assert_check(
        "and monotonic through it",
        all(b >= a - 1e-9 for a, b in pairwise(samples)),
    )
...
def test_tile_footprint_chunk_filtering():
    from geotessera.zarr import (
        GLOBAL_BASE_RES,
        GLOBAL_BOUNDS,
        GLOBAL_CHUNK,
    ...
    _west, _, _, _north = GLOBAL_BOUNDS

    def _chunk_of(lon, lat):
        return (
            int((_north - lat) / GLOBAL_BASE_RES) // GLOBAL_CHUNK,
            int((lon - _west) / GLOBAL_BASE_RES) // GLOBAL_CHUNK,
        )

    one = chunks_for_tile_centres([(0.05, 52.05)])
    assert_check("a tile yields at least one chunk", len(one) >= 1)
    assert_check(
        "and includes the chunk under its centre", _chunk_of(0.05, 52.05) in one
    )
    ...
    assert_check("a tile spans a plausible number of chunks", 4 <= len(one) <= 12)

    assert_check("no tiles yields no chunks", chunks_for_tile_centres([]) == set())
    ...
    assert_check(
        "adjacent tiles share at most an edge of chunks", len(adj) < 2 * len(one)
    )

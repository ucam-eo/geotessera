...
group. The store automatically routes geographic queries to the correct zone::
    ...
    zarr store
    ├── Root attributes (geoemb:model, geoemb:depths, ...)
    ├── utm30/               # UTM Zone 30
    │   ├── time             # Year coordinate array
    │   ├── x, y             # UTM pixel-centre coordinates
    │   ├── embeddings       # (time, band, y, x) int8
    │   ├── embeddings_d4    # First 4 dimensions (v2 stores)
    │   ├── embeddings_d16   # First 16 dimensions (v2 stores)
    │   └── scales           # (time, y, x) float32
    ├── utm31/               # UTM Zone 31
    │   └── ...
    └── ...

The real value is ``embeddings * scales``; ``scales`` carries two
sentinels, NaN for water and +inf for a pixel never written. The
``geoemb:depths`` attribute names the matryoshka prefix arrays a v2
store carries.
...
- **Region reading**: ``read_region()`` for loading rectangular areas as
  mosaics with CRS and transform metadata; ``read_region_quantized()``
  returns the int8 values and scales instead, at a quarter of the bytes
...
- **Depths**: ``depth=`` on the read methods selects a matryoshka prefix
  array on stores that declare one
...
**Transport**: http(s) reads go through ``obstore``, which retries each
request with exponential backoff and jitter, so one dropped response
from a busy server costs a chunk rather than the read. The store can be
wrapped — ``zarr_store()`` builds the default one — to add caching or
other behaviour; see :doc:`zarr_quickstart`.
...
- ``cross_zone`` also tries the neighbouring zone within 0.1 degrees of a seam
